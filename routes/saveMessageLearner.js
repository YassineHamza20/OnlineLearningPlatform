...
router.post( '/saveMessages', auth, roleCheck(['Learner']), isSubscribed, (req, res) =>{
    ...
    const uuid = uuidv4();
    const queryy ="SELECT id, firstname, lastname, pfp from tutor where uuid=?"
    mysql.query(queryy, [id], (err, resul)=> {
        if (err) {
            ...
            mysql.query(query, [uuid,userId,resul[0].id,msg,role], (error, result)=> {
                if (error) {
                    console.log(error);
                    res.status(500).json(error);
                }
                else {
                    const query= `select pfp from learner where id = ?`
