...
function Notifications(props) {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            try {
                setLoading(true)
                const notifications = await axiosInstance.post('${process.env.REACT_APP_BACKEND_URL}/tutor/getNotifications', {
                    page: 1,
                    pageSize: 5,
                    Accepted: ""
                }, {
      ...
      const handleContent = () => {
        ...
        const handleMarkAllAsRead = async () => {
            try {
                ...
                const reponse = await axiosInstance.post('${process.env.REACT_APP_BACKEND_URL}/tutor/markAllAsRead', {
