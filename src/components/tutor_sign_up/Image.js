...
//             try {
//                 const response = await axiosInstance.post(`${process.env.REACT_APP_SERVER_URL}/imageFaceDetection', formData, {
//                     headers: {
    ...
    const handleImageChange = async (event) => {
        ...
        if (event.target.files.length > 0) {
            ...
            try {
                ...
                // Always proceed with image upload regardless of face detection
                reader.readAsDataURL(imageFile);
                dispatch(setServerImage(imageFile));
                reader.onload = () => {
                    dispatch(setDisplayableImage(reader.result));
