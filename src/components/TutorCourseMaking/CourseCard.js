...
function CourseCard(props) {
    ...
    return (
        <>
            <div className="relative group flex flex-col w-56 min-h-96 max-h-96 space-y-6 pb-3 rounded-3xl border hover:bg-lightg">
                <img 
                ...
                className="absolute transition-all duration-200 cursor-pointer hidden group-hover:block left-2 p-2 hover:text-errortext hover:border-errortext border border-errortext hover:bg-errorbg  rounded-full bg-errortext text-white top-[-20px]">
            ...
            {
                visibility?
                <DeleteCourseModal courseId={props.course.id} setVisibility={setVisibility}></DeleteCourseModal>
                :
                null
            }
        </>
