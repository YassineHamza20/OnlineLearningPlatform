...
function WorkExperience(props) {
   ...
   const Tags = [
      ...
      const handleClick = async () => {
        ...
        const handleSave = async (e) => {
            ...
            if(editing) {//if he's in editing mode
                if(!(value.some(item => item.title === null || item.title === "" || item.description === null || item.description === "")||  areListsEqual(value, WorkExperience))) {
                    try {
                        await props.modifyCall(value, 'W.E')
                        dispatch(setWorkExperience(value))
                    } catch (error) {
                        console.log(error);
                    }
                }else {
        ...
        const handleWorkExperienceTitleChange = (e, wxp) => {
        ...
        const handleWorkExperienceDescriptionChange = (e, wxp) => {
        ...
        const handleWorkExperienceTagChange = (e, wxp) => {
        ...
        const handleInput=() => {
            const workExpInput = value.map((wxp, index) => {
                ...
                return <div key={index} className="flex flex-col space-y-3 relative w-full justify-center">
                    ...
                    <input 
                    onChange={(e) => handleWorkExperienceTitleChange(e, wxp)} 
                    ...
                    <select
                        ...
                        name="languages" value={wxp.tag} onChange={(e) => handleWorkExperienceTagChange(e, wxp)}
                    >
                ...
                <textarea onChange={(e) => handleWorkExperienceDescriptionChange(e, wxp)} value={wxp.description} placeholder='Write something about it...' className="w-[70%] min-h-20 appearance-none bg-white border border-gray-300 hover:border-gray-500 px-4 py-2 rounded shadow leading-tight focus:outline-none focus:shadow-outline resize-none"></textarea>
                </div>
    ...
    console.log("condition:" , value.some(item => item.title === null || item.title === "" || item.description === null || item.description === ""));
    return (
